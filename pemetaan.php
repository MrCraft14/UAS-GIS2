...
<div class="page-content p-5" id="content">
    <button class="btn btn-danger bg-danger rounded-pill shadow-sm px-4 mb-4" id="sidebarCollapse" type="button">
            ...
            <div class="form-row">
                <div class="form-group col-md-6">
                    ...
                    <select id="lokasi_awal" class="form-control">
                        ...
                        <?php
                        include 'koneksi.php';
                        $kec = $conn->query("SELECT * FROM marker");
                        if ($kec->num_rows > 0) {
...
<script>
    ...
    if ($hasil->num_rows > 0) {
        while ($row = $hasil->fetch_row()) { ?>
            L.marker([<?= $row[5] ?>, <?= $row[6] ?>]).bindPopup('Nama : <?= $row[2] ?> <br> Alamat : <?= $row[3] ?> <br>' +
                "<a href='detail_marker.php?id=<?= $row[0] ?>' class='btn btn-outline-info btn-sm'>Detail</a>").addTo(map);
    <?php }
    }
    ?>
    <?php
